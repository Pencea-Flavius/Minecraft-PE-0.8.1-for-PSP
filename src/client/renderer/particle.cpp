...
void tickOne(World* w, P* p) {
    ...
    case K_MOBFLAME: {
        p->yOff += p->rise;
        if (p->entityId) {
            Entity* e = g_level.getEntity(p->entityId);
            if (!e || e->removed || e->onFire < 2 || !e->isAlive()) {
                p->entityId = 0;
            } else if (e->isOnFire()) {
                p->rise += e->bbHeight * 0.002f;
                p->x = e->x + p->xOff; p->y = e->y + p->yOff; p->z = e->z + p->zOff;
            } else {
                p->active = false;
            }
        } else {
...
void particlesEntityFlame(Entity* e) {
    static int s_tick = 0;
    const float rx = e->bb.x0 + frand() * (e->bb.x1 - e->bb.x0);
    const float ry = e->bb.y0 + frand() * (e->bb.y1 - e->bb.y0);
    const float rz = e->bb.z0 + frand() * (e->bb.z1 - e->bb.z0);

    if ((s_tick & 1) && !tooFar(rx, ry, rz)) {
        float lift = (ry < e->y) ? 1.0f : 0.0f;
        P* p = alloc();
        if (p) { baseInit(p, rx, ry + lift, rz, 0, 0, 0);
            p->kind = K_SMOKE;
            p->xd = p->xd * 0.1f + e->xd * 0.5f;
            p->yd *= 0.1f;
            p->zd = p->zd * 0.1f + e->zd * 0.5f;
            p->r = p->g = p->b = frand() * 0.5f; p->size *= 0.75f; p->oSize = p->size;
    ...
    if (++s_tick == 10) {
        s_tick = 0;
        if (tooFar(rx, e->bb.y0, rz)) return;
        P* p = alloc(); if (!p) return;
        baseInit(p, rx, e->bb.y0, rz, 0, 0, 0);
        p->kind = K_MOBFLAME;
        p->noPhysics = true;
        p->fullBright = true;
        ...
        const float half = e->bbWidth * 0.5f;
        p->y += 0.7f;
        p->xOff = (p->x - e->x) * half + (frand() * 2.0f - 1.0f) * 0.1f;
        p->yOff = p->y - e->y;
        p->zOff = (p->z - e->z) * half + (frand() * 2.0f - 1.0f) * 0.1f;
        p->x = e->x + p->xOff; p->y = e->y + p->yOff; p->z = e->z + p->zOff;
        p->rise = e->bbHeight * 0.5f * 0.01f;
        tickOne(0, p);
    }
}
...
void particlesRender(World* w, float yawDeg, float pitchDeg, float alpha,
    ...
    for (int i = 0; i < MAX_PARTICLES; i++) {
        ...
        if (g_pool[i].kind == K_MOBFLAME) nFire++;
    ...
    static Texture s_fireAtlas;
    static int s_fireState = 0;
    if (nFire && !s_fireState)
        s_fireState = textureLoad16("data/images/fire_atlas.png", &s_fireAtlas, GU_PSM_5551) ? 1 : -1;
    const Texture* fire = s_fireState > 0 ? &s_fireAtlas : 0;
    ...
    for (int pass = 0; pass < 4; pass++) {
        ...
        for (int i = 0; i < MAX_PARTICLES; i++) {
            ...
            int pp = p->kind == K_MOBFLAME ? 3 : p->itemsAtlas ? 2 : p->terrainAtlas ? 1 : 0;
            if (pp != pass) continue;
            ...
            float u0, u1, v0, v1;
            if (p->kind == K_MOBFLAME) {
                const float HU = 1.0f / 32.0f, HV = 1.0f / 1024.0f;
                int frame = (int)floorf(sfrac * 32.0f);
                if (frame > 31) frame = 31;
                if (frame < 0) frame = 0;
                u0 = HU; u1 = 1.0f - HU;
                v0 = frame / 32.0f + HV; v1 = (frame + 1) / 32.0f - HV;
