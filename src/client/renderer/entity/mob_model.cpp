...
float mobFireOverlayAlpha(Entity* e) {
    if (!e->isOnFire()) return 0.0f;
    float v = powf((float)e->onFire / 100.0f * 0.5f, 0.3f);
    return (v + 0.25f + 0.1f) * 0.6f;
}

void mobFireOverlayBegin(float alpha) {
    const unsigned int ORANGE = 0x000041CCu;
    unsigned int a8 = (unsigned int)(alpha * 255.0f); if (a8 > 255) a8 = 255;
    sceGuEnable(GU_BLEND);
    sceGuBlendFunc(GU_ADD, GU_SRC_ALPHA, GU_ONE_MINUS_SRC_ALPHA, 0, 0);
    sceGuTexFunc(GU_TFX_BLEND, GU_TCC_RGBA);
    sceGuTexEnvColor(ORANGE);
    sceGuColor((a8 << 24) | ORANGE);
    sceGuDepthMask(GU_TRUE);
    sceGuEnable(GU_CULL_FACE);
    sceGuFrontFace(GU_CCW);
}

void mobFireOverlayEnd() {
...
void mobDrawPartPlain(const MobVertex* base) {
    float ovm[16], oMin = 0.0f, oSafe = 0.0f;
    if (!partNear(base, ovm, &oMin, &oSafe)) {
        sceGumDrawArray(GU_TRIANGLES, MOB_FMT, 36, 0, base);
    } else {
        for (int f = 0; f < 6; f++) drawFace(base + f * 6, ovm, oMin, oSafe);
    }
}
...
void mobRenderParts(Mob* mob, MobPart* parts, int count, Texture* tex,
    ...
    const float fireA = mobFireOverlayAlpha(mob);
    if (fireA <= 0.0f && (mob->hurtTime > 0 || mob->deathTime > 0)) {
