...
static void lastPlayedString(long secs, char* out, int cap) {
    if (secs < 86400)        snprintf(out, cap, "Today");
    else if (secs < 172800)  snprintf(out, cap, "Yesterday");
    else if (secs < 604800)  snprintf(out, cap, "%ld days ago", secs / 86400);
    else if (secs < 2419200) snprintf(out, cap, "%ld weeks ago", secs / 604800);
    else                     snprintf(out, cap, "long ago");
}
...
void worldListScan(WorldList* out) {
    ...
    while (sceIoDread(d, &entry) > 0 && out->count < MCPSP_MAX_WORLDS) {
        if (FIO_S_ISDIR(entry.d_stat.st_mode) &&
            ...
            snprintf(out->dates[out->count], sizeof(out->dates[0]), "long ago");
    ...
    for (int i = 0; i < out->count; i++) {
        ...
        if (ok) {
            ...
            if (sceRtcGetCurrentClockLocalTime(&cur) >= 0 &&
                sceRtcGetTick(t, &then) >= 0 && sceRtcGetTick(&cur, &now) >= 0)
                lastPlayedString(now > then ? (long)((now - then) / 1000000ull) : 0,
                                 out->dates[i], sizeof(out->dates[0]));
        }
    }
...
bool worldListCreate(WorldList* list, const char* inName, char* outName, int gamemode, long seed,
    ...
    snprintf(list->dates[list->count], sizeof(list->dates[0]), "Today");
