...
static int creatureYNear(Level* L, int x, int z, int ) {
    return L->getTopSolidBlock(x, z);
}
...
static void spawnCreatures(Level* level) {
    ...
    for (int attempt = 0; attempt < SPAWN_ATTEMPTS; attempt++) {
        ...
        if (!biomeHasCreatures(worldBiomeAt(worldSeed(), bx, bz))) continue;
...
void populateInitial(Level* level) {
    ...
    for (int oi = 0; oi < NCHUNKS; oi++) {
        ...
        if (!biomeHasCreatures(worldBiomeAt(worldSeed(), xo + 16, zo + 16))) continue;
