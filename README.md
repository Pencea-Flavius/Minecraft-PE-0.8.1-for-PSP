...
This repository contains a recreation of **Minecraft Pocket Edition v0.8.1** for the Sony PlayStation®Portable.

It runs on **every PSP™ model, including the 32 MB PSP™-1000** — see [Hardware](#hardware) for what the 1000 gives up.  
All *World Logic*, *Save Format*, *Block Interaction*, *Movement* etc. are as in **v0.8.1**[^2]
on your phones[^1].  
### This includes:  
* world generation,
* gamemodes,
...
The port does not require the plugin and never refuses to run without one:  
...
The other way around, open a world made here in **v0.8.1** — the port uses
the 0.8.1 block set, and an older Pocket Edition simply does not know those
...
- Nothing here writes outside its own folder, and nothing touches the original firmware, **however**,  
...
[^2]: https://minecraft.wiki/w/Pocket_Edition_v0.8.1_alpha
